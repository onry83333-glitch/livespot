-- ============================================================
-- create_dm_batch: DM一斉送信キュー登録 RPC
-- フロントエンドの sb.rpc('create_dm_batch', ...) から呼び出される
-- ============================================================
...
BEGIN
  ...
  LOOP
    INSERT INTO public.dm_send_log (
      account_id, user_name, message, status, campaign, template_name
    ) VALUES (
      p_account_id, v_target, p_message, 'queued', v_batch_id, COALESCE(p_template_name, '')
    );
    v_count := v_count + 1;
  END LOOP;
